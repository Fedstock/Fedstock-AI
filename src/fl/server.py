...
class BubbleServer:
    ...
    def load_clustering_results(self, clustering_json_path):
        """
    ...
    def step_4_personalized_learning(self, epochs=10):
        """
        Performs local training only (Personalized FL) for isolated clients.
        """
        if not self.isolated:
            print("\nNo isolated clients to process.")
            return
            
        print("\n--- PA-CFL Step 4: Personalized Learning for Isolated Clients ---")
        for cid in self.isolated:
            if cid not in self.clients:
                continue
            print(f"\n[Personalized] Starting local training for client: {cid}")
            client = self.clients[cid]
            ...
            print(f"    -> Client {cid} metrics after {epochs} epochs: RMSE={metrics['rmse']:.4f}, SMAPE={metrics['smape']:.4f}")
